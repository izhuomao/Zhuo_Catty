...
void Check_Start(){
    ...
    if(bsum>=Threshold_HIGH&&record_count==0){ //开始统计
      Serial.println("开始");
      smooth_count=0;
    }

    if(smooth_count<=10){ //声音统计中
      if(bsum>Threshold_LOW) //清除疑似没人说话的记录
        {
          smooth_count=0;
        }
        record_count++;
    ...
    if(bsum <= Threshold_LOW)//检测到疑似没人说话
      {
        if(smooth_count>=10&&record_count>0) //话音刚落结束统计
        {
            Serial.println("检测完成");
            Serial.print("收集到的元组数为：");
            Serial.println(record_count-10); //最后10条数据没有实际效果
            record_count=0;
        }
        smooth_count++;
      }
